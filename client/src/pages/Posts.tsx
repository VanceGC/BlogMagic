          ...
          {!posts || posts.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredAndSortedPosts.map((post) => {
                ...
                const isOrphaned = (post as any).isOrphaned;

                return (
                  <Card 
                    key={post.id} 
                    className={`hover:shadow-lg transition-shadow cursor-pointer ${
                      isOrphaned ? "border-2 border-orange-300 bg-orange-50" : ""
                    }`}
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          {/* Blog badge or orphaned warning */}
                          {isOrphaned ? (
                            <div className="mb-3">
                              <span className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full font-medium bg-orange-200 text-orange-800">
                                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                  <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                                </svg>
                                Unassigned - Blog Deleted
                              </span>
                            </div>
                          ) : blogConfig ? (
                            <div className="mb-3">
